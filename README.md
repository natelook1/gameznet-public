...
The **WoW** tab is a full guild dashboard: roster, character profiles, progression tracking
(vault/keystones/lockouts), a professions materials calculator, and live combat log pulls.
Most of it works from a Battle.net link alone; the optional bundled **GamezNET addon** adds
what the Blizzard API doesn't expose — bags, bank, housing, and combat log pulls.

**→ See [`docs/WOW.md`](docs/WOW.md) for the full player guide** — every tab, addon setup,
combat log commands, privacy controls, and how to read a Pulls card.
...
* **Container Orchestration:** Deployed as a multi-node replicated microservice layer inside an orchestrated **Docker Swarm cluster** running on high-performance Linux VMs.
* **Hypervisor Fabric:** Hosted across a **3-node Proxmox VE HA (High Availability) Cluster** featuring automatic corosync node failovers and split-brain mitigation.
* **Automated Router Provisioning:** Integrates an automated server-side SSH pipeline that synchronizes peer configurations directly with a **Ubiquiti UniFi Dream Machine (UDM)**, performing automated configuration purges of orphaned tokens.
* **Ingress & Perimeter Edge:** All public-facing domains (`*.looknet.ca`) route through a zero-trust **Cloudflare Tunnel mesh**, completely eliminating the need for inbound open firewall ports, with internal routing handled by **Traefik v3** clusters acting as reverse proxies.

### 🛠️ Build Engineering & Deployment Pipeline

The Windows application is compiled locally into a standalone native executable to bypass heavy Electron/Web-View performance constraints.

```bash
python build.py
```

* **PyInstaller Compilation:** Compiles the local Python/Flask execution loop utilizing a localized `onedir` deployment spec file to preserve runtime library links without temp-extraction execution penalties.
* **Inno Setup Packaging:** Wraps the payload bundle, assets, and dependency logic into a silent installer using **Inno Setup 6**.
* **Artifact Deployment:** An automated PowerShell script (`release.ps1`) patches the versioning schema, executes the build, and publishes the validated binary payload to server-side artifact repositories.

### 📋 Environment Variable Matrix

All variables are loaded from `/etc/gameznet/.env` at deploy time.

| Variable | Description |
| :--- | :--- |
| `ADMIN_PASSWORD` | Secure hash for the administrative portal |
| `PTERODACTYL_API_KEY` | Client API key for game server state control |
| `YOUTUBE_API_KEY` | Google Cloud API key for proxy caching |
| `DISCORD_BOT_TOKEN` | Discord application Bot Token |
| `STEAM_API_KEY` | Steam Web API Key |
| `SERVER_ENDPOINT_IP` | Public WireGuard gateway target |

### 🔌 Core API Endpoint Architecture

| Endpoint | Method | Access Scope | Functional Responsibility |
| :--- | :--- | :--- | :--- |
| `/api/server-config` | GET | Cryptographic Token | Exposes signed WireGuard parameters for the peer-to-peer mesh. |
| `/api/heartbeat` | POST | Authenticated Client | Accepts live telemetry, pings, and heuristic game status IDs. |
| `/api/conan-exiles/stream` | GET | Internal Mesh | SSE stream for real-time structural and player world-state vectors. |
| `/api/files/upload` | POST | Authenticated ACL | Manages identity-scoped multiparty file allocations. |
| `/admin/wg/sync` | POST | Root Administrator | Synchronizes infrastructure state across the physical UDM gateway. |

### 🔒 Security Posture

* **Cryptographic Authorization:** All internal routing mandates unique, single-redemption cryptographic tokens mapped cleanly against dedicated server-side WireGuard identities.
* **Side-Channel Timing Protections:** Leverages constant-time algorithmic comparisons (`crypto.timingSafeEqual`) for all token confirmation pipelines to eradicate the threat of structural timing attacks.
* **Network Isolation:** Sensitive APIs, SSH orchestration routines, and storage drops are strictly scoped to clients validated from within the WireGuard cryptotunnel.
