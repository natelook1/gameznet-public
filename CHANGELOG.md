...
### Added
- **Player Estate (housing)** — replaces the "not yet available via the Blizzard API" placeholder, which was outdated once Blizzard shipped housing APIs in 11.2.7. Decor collections come from `/profile/.../collections/decor`; owned houses, neighborhood and house favor come from a new `C_Housing` addon capture, since the Web API exposes no house data (`/profile/.../house/{id}` 404s). Renders in all three paths: the Preact card, the vanilla-JS block in `templates/index.html`, and mobile via `cf-pages/wow.js`. Addon bundle 1.1.2 → 1.2.0 and schema 2 → 3. The bundle version must move whenever the Lua changes: the installer only pushes files when `installed_ver != ADDON_VERSION`, so leaving it would have meant existing clients never received the new capture, while the mismatch handler told them to `/reload` — advice that cannot help when the new Capture.lua was never installed.
